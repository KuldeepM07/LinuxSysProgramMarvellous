...
int main(int argc, char *argv[])
{
    ...
    while((directoryEntry = readdir(directorySource)) != NULL)
    {
        ...
        if(S_ISREG(sObjFileStat.st_mode))
        {
            ...
            strcpy(uObjFileInfo.cFileName, directoryEntry->d_name);
            uObjFileInfo.iFileSize = sObjFileStat.st_size;

            write(iFDDestinationFile, &uObjFileInfo, sizeof(uObjFileInfo));

            // Write contents of file to file
            iFDOfSourceFileForReading = 0;
            iFDOfSourceFileForReading = open(cFileName, O_RDONLY);
            if(iFDOfSourceFileForReading != -1)
            {
                memset(cBufferForReadingBytes, '\0', BLOCK_SIZE);
                while((iRetValReadBytes = read(iFDOfSourceFileForReading, cBufferForReadingBytes, BLOCK_SIZE)) != 0)
                {
                    // Write to all combined files
